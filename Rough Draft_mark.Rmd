...
```{r Comments per View, echo=FALSE}
youtube %>%
  group_by(category_id) %>%
  summarise(total_comments = sum(comment_count),
...
youtube %>% 
  ...
  scale_fill_discrete(name = "Published Country")
```

### Have trending videos seen a change in their usage of capital letters?
```{r more Capital Letters, echo=FALSE}
youtube %>% 
  distinct(video_id, .keep_all = TRUE) %>% 
...
### Is there a discernible trend in the the length of trending video titles?
```{r, echo=FALSE}
youtube %>% 
  ...
  labs(x = "Title Length",
       y = "Number of Trending Videos",
       title = "Title Length in Trending Video Titles",
       caption = "Calculated with distinct video ID's") +
  theme(text = element_text(family = "Times New Roman", color = "grey20"))
```
...
ggplot(youtube_time, aes(x=hour, fill=publish_country))+
 ...
 labs(x="Hour", y="Proportion of Total Distinct Videos Published By Country", title="Distribution of Times \nWhen Trending Videos are Published")
```
...
youtube %>% 
  ...
  group_by(category_id) %>% 
  summarise(median_days_return = median(trend_diff))
youtube %>% 
  group_by(video_id) %>% 
  mutate(trend_diff = trending_date - lag(trending_date, order_by = trending_date)) %>% 
  select(video_id, trend_diff, trending_date, category_id) %>%
  arrange(desc(trend_diff)) %>% 
  distinct(video_id, .keep_all=TRUE) %>% 
  filter(trend_diff > 1) %>% 
  filter(category_id == "Entertainment" | category_id == "Music" |
...
youtube %>% 
  ...
  group_by(publish_country) %>% 
  summarise(median_days_trending = median(days_trending))
youtube %>% 
  group_by(video_id) %>% 
  mutate(days_trending = n()) %>% 
  distinct(video_id, .keep_all=TRUE) %>% 
